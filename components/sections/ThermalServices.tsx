...
function ThermalCard({ service }: { service: any }) {
  ...
  return (
    ...
    >
      <div className="relative h-[250px] w-full overflow-hidden bg-black cursor-[url('data:image/svg+xml;utf8,<svg xmlns=\\'http://www.w3.org/2000/svg\\' width=\\'20\\' height=\\'20\\'><circle cx=\\'10\\' cy=\\'10\\' r=\\'8\\' fill=\\'rgba(255,100,0,0.5)\\' stroke=\\'white\\' stroke-width=\\'2\\'/></svg>'),_auto]">
        {/* Base Image (Cold/Grayscale) */}
        <img 
          src={service.image} 
          alt={service.title} 
          className="absolute inset-0 w-full h-full object-cover grayscale opacity-40 transition-opacity duration-500 group-hover:opacity-20"
        />
        
        {/* Thermal Color Overlay mapped to mouse position using radial gradient mask */}
        <motion.div
          className="absolute inset-0 w-full h-full pointer-events-none z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          style={{
        ...
        {/* Revealed Full Color Image */}
        <motion.div
          className="absolute inset-0 w-full h-full overflow-hidden opacity-0 group-hover:opacity-100 transition-opacity duration-300"
          style={{
            WebkitMaskImage: useMotionTemplate`
              radial-gradient(
      ...
      <div className="relative p-8 bg-white z-20">
        <div className="flex items-center gap-4 mb-4">
          <div className="p-3 bg-ivory rounded-lg text-terracotta transition-transform duration-300 group-hover:scale-110 group-hover:bg-terracotta/10">
            <service.icon size={24} />
          </div>
          <h3 className="text-2xl font-serif text-navy group-hover:text-terracotta transition-colors">{service.title}</h3>
        </div>
        <p className="text-navy/60 font-sans font-light leading-relaxed">
          {service.description}
        </p>
        <a href="#contact" className="inline-flex items-center gap-2 mt-6 font-sans text-sm font-semibold text-terracotta opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all duration-300">
...
export default function ThermalServices() {
  return (
    <section id="services" className="py-24 lg:py-32 bg-ivory relative" data-nav-theme="light">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16 md:mb-24">
          <span className="text-terracotta uppercase tracking-[0.3em] font-sans text-xs font-bold mb-4 block">
            What We Do
          </span>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif text-navy">
            We see the invisible.
          </h2>
          <p className="mt-6 text-navy/60 max-w-2xl mx-auto font-light font-sans text-lg">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          {SERVICES.map((service, i) => (
            <ThermalCard key={i} service={service} />
          ))}
        </div>
      </div>
    </section>
